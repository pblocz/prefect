...
| [Deployment flow run limits](/v3/concepts/deployments#concurrency-limiting) | Specific deployment | Cap concurrent runs of a specific deployment (lease-based; lease behavior is not user-configurable) |
...
<Note>
Deployment flow run limits also rely on timed leases, but their lease duration and renewal behavior are managed
by Prefect and cannot be configured. A run whose lease renewal fails is crashed (`410 Gone`) with no
`raise_on_lease_renewal_failure` escape hatch. To serialize long-running flows, prefer global concurrency
limits, where you can tune the lease duration and tolerate transient renewal failures. See
[How deployment concurrency is enforced](/v3/concepts/deployments#how-deployment-concurrency-is-enforced).
</Note>
