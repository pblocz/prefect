...
Automatic installation and `EXTRA_PIP_PACKAGES` cover Python packages only. If a Python package depends on a
system-level library (for example, `python-magic` needs `libmagic`), bake that library into a custom image. See
[Install system dependencies in a custom image](/v3/how-to-guides/deployment_infra/docker/#install-system-dependencies-in-a-custom-image).
