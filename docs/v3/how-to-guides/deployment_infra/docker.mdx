...
If you already install your project's dependencies at build time (for example with `uv pip install --system`),
leave automatic dependency installation off. Enabling it when a `pyproject.toml` is present in the working
directory can cause `uv` to build a fresh environment and reinstall dependencies on every flow run, which may
trigger out-of-memory errors or slow cold starts. Keeping `pyproject.toml` out of the runtime working directory
avoids this. See
